...
def parse_arguments():
    ...
    parser.add_argument(
        "--enable_fp8", default=False, action="store_true", help="Use FP8 Linear layer for Attention QKV/Dense and MLP."
    )
    parser.add_argument(
        "--fp8_kv_cache",
        default=False,
    ...
    parser.add_argument(
        "--quantized_fp8_model_path", type=str, default=None, help="Path of a quantized model checkpoint in .npz format"
    )
    parser.add_argument(
        "--use_weight_only",
        default=False,
    ...
    parser.add_argument(
        "--tokens_per_block", type=int, default=128, help="Number of tokens per block in paged KV cache"
    )
    parser.add_argument(
    ...
    if args.model_dir is not None:
        ...
        # adjust the n_positions based on the given max_input_len arg
        # original code:
        # args.n_positions = hf_config.max_position_embeddings
        # after modification:
        args.n_positions = args.max_input_len + args.max_output_len
...
def build_rank_engine(builder: Builder, builder_config: tensorrt_llm.builder.BuilderConfig, engine_name, rank, args):
    """
    ...
    mapping = Mapping(world_size=args.world_size, rank=rank, tp_size=args.tp_size, pp_size=args.pp_size)
    ...
    # Initialize Module
    tensorrt_llm_llama = swiftinfer.models.LLaMAForCausalLM(
    ...
    if args.use_smooth_quant or args.use_weight_only:
        ...
        quant_scales = get_scaling_factors(
            args.quantized_fp8_model_path, num_layers=args.n_layer, quant_mode=args.quant_mode
        )
        quantize_kwargs = {"quant_scales": quant_scales}
    tensorrt_llm_llama = quantize_model(tensorrt_llm_llama, args.quant_mode, **quantize_kwargs)
    if args.per_group:
        ...
        load_from_meta_llama(tensorrt_llm_llama, args.meta_ckpt_dir, mapping, args.dtype)
        ...
        if not args.load_by_shard:
            ...
            load_from_hf_checkpoint(
                tensorrt_llm_llama, args.model_dir, mapping, dtype=args.dtype, lora_config=args.lora_config
            )
        tok = time.time()
        t = time.strftime("%H:%M:%S", time.gmtime(tok - tik))
    ...
    with net_guard(network):
        ...
        inputs = tensorrt_llm_llama.prepare_inputs(
            ...
            max_kv_cache_size=args.max_input_len + args.max_output_len,
        )
        tensorrt_llm_llama(*inputs)

        # mark as TRT network output
        # ----------------------------------------------------------------
        # for k, v in tensorrt_llm_llama.layers[0].attention.named_network_outputs():
